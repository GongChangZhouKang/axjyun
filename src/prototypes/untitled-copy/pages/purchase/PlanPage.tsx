    ...
    return (
        <>
            ...
            <section className="panel">
                <SectionTitle
                    title="集团采购计划"
                    subtitle="计划阶段记录预算价格，订单生成后对比订单价格"
                />
                <div className="table-toolbar">
                    ...
                    <button type="button" className="secondary-btn" onClick={() => openWindow({
                        ...
                        primary: '确认导出',
                        body: <DetailGrid rows={[['导出范围', '当前筛选条件'], ['包含字段', '计划单号、采购模式、预算金额、订单金额、价差、审核状态、订单生成状态'], ['用途', '采购审批、价格差异复核']]} />,
                    })}>导出</button>
                </div>
                <DataTable
                    columns={['计划单号', '采购模式', '需求来源', '明细数', '采购内容', '数量合计', '预算金额', '订单金额', '价差', '审核状态', '关联订单数', '操作']}
                    rows={plans}
                    renderRow={(row: PlanRow) => {
                        const relatedOrderCount = relatedOrdersForPlan(row, orderRows).length;
                        const hasOrderAmount = relatedOrderCount > 0;
                        ...
                        return (
                            <tr key={row.code}>
                                ...
                                <td>{hasOrderAmount ? currency(planOrderAmountFromOrders(row, orderRows)) : '--'}</td>
                                <td>{hasOrderAmount ? planOrderVarianceFromOrders(row, orderRows) : '--'}</td>
                                <td><StatusTag value={row.auditStatus} /></td>
                                <td>
                                    {relatedOrderCount > 0 ? (
                                        <button type="button" className="table-link" onClick={openOrders}>
                                            {relatedOrderCount} 单
                                        </button>
                                    ) : '0 单'}
                                </td>
