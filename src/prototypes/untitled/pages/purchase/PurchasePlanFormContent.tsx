...
export function PurchasePlanFormContent({
    ...
    return (
        <>
            ...
            {isView && row && (
                <CreateSection title="关联采购订单" subtitle="一个采购计划可分多批导入订单，订单导入后继续按仓库和采购需求分配入库">
                    <PlanOrderRelationTable plan={row} openOrderDetail={openOrderDetail} onOpenTrace={onOpenTrace} />
                </CreateSection>
            )}

            <CreateSection title="流程" subtitle="采购计划提交后先走内部审核，审核通过后再进入订单导入">
